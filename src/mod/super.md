...
Ключевые слова `super` и `self` могут быть использованы для того, 
чтобы устранить неоднозначность между используемыми элементами а также для того, чтобы уменьшить количество кода-описания пути
к содержанию модуля.
...
mod my {
    ...
    pub fn indirect_call() {
        ...
        // В нашем случае - это модуль `my`.
        // Вызов `self::function()`, так же как и вызов `function()` даёт одинаковый результат,
